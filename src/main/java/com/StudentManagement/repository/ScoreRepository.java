...
public interface ScoreRepository extends JpaRepository<Score, Long> {
        ...
        @EntityGraph(attributePaths = { "student", "student.user", "subject" })
        @Query("select sc from Score sc where sc.subject.id = :subjectId")
        ...
        // Lấy tất cả điểm theo lớp học (không phân trang)
        @EntityGraph(attributePaths = { "student", "student.user", "subject" })
        @Query("select sc from Score sc join sc.student st where st.classroom.id = :classroomId")
        List<Score> findAllByClassroomId(@Param("classroomId") Long classroomId); // Lấy điểm theo lớp và môn học
        ...
        // Lấy điểm theo danh sách lớp và môn học cụ thể
        @EntityGraph(attributePaths = { "student", "student.user", "subject" })
        @Query("select sc from Score sc join sc.student st where st.classroom.id in :classroomIds and sc.subject.id = :subjectId")
        List<Score> findByClassroomIdsAndSubjectId(@Param("classroomIds") List<Long> classroomIds,
                        @Param("subjectId") Long subjectId);

        // Lấy tất cả điểm theo danh sách lớp
        @EntityGraph(attributePaths = { "student", "student.user", "subject" })
