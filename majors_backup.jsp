                ...
                <body>
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function () {
                            ...
                            // Setup debug panel
                            const testJSBtn = document.getElementById('testJS');
                            if (testJSBtn) {
                                testJSBtn.onclick = function() {
                                    debugLog('✅ JavaScript is working!');
                                    alert('JavaScript OK!');
                                };
                            }
                            
                            const testButtonsBtn = document.getElementById('testButtons');
                            if (testButtonsBtn) {
                                testButtonsBtn.onclick = function() {
                                    debugLog('=== BUTTON ANALYSIS ===');
                                    ...
                                    debugLog(`📈 Total DOM elements: ${allElements.length}`);
                                    ...
                                    allButtons.forEach((btn, index) => {
                                        debugLog(`  Button ${index}: class="${btn.className}" id="${btn.id}"`);
                                    });
                                    ...
                                    const btnOutlineDanger = document.querySelectorAll('.btn-outline-danger');
                                    
                                    debugLog(`�️ Delete buttons (.delete-subject-btn): ${deleteButtons.length}`);
                                    debugLog(`✏️ Edit buttons (.edit-subject-btn): ${editButtons.length}`);
                                    debugLog(`🟠 Outline danger buttons (.btn-outline-danger): ${btnOutlineDanger.length}`);
                                    
                                    // Check table existence
                                    const tables = document.querySelectorAll('table');
                                    const tbody = document.querySelectorAll('tbody');
                                    debugLog(`📋 Tables found: ${tables.length}`);
                                    debugLog(`📋 Table bodies found: ${tbody.length}`);
                                };
                            }
                            
                            const checkPageBtn = document.getElementById('checkPage');
                            if (checkPageBtn) {
                                checkPageBtn.onclick = function() {
                                    debugLog('=== PAGE CONTEXT CHECK ===');
                                    ...
                                    const hasMainContent = document.querySelector('.container-fluid') !== null;
                                    const hasSubjectTable = document.querySelector('table') !== null;
                                    const hasSubjectRows = document.querySelectorAll('tbody tr').length;
                                    
                                    debugLog(`🏗️ Has main container: ${hasMainContent}`);
                                    debugLog(`📋 Has subject table: ${hasSubjectTable}`);
                                    debugLog(`📊 Subject rows count: ${hasSubjectRows}`);
                                    
                                    // Check for specific content indicators
                                    const hasSubjectCodeColumn = document.querySelectorAll('th').length > 0;
                                    const hasActionColumn = document.querySelector('th') !== null;
                                    
                                    debugLog(`🏷️ Has table headers: ${hasSubjectCodeColumn}`);
                                    debugLog(`⚙️ Has any th elements: ${hasActionColumn}`);
                                    
                                    // Check for the "choose major" message
                                    const hasChooseMajorMessage = document.querySelector('h5') !== null;
                                    const chooseMajorText = document.querySelector('h5') ? document.querySelector('h5').textContent : 'none';
                                    debugLog(`🎯 Has h5 element: ${hasChooseMajorMessage}`);
                                    debugLog(`� H5 text content: "${chooseMajorText}"`);
                                    
                                    // Check right panel state
                                    const rightPanelContent = document.querySelector('.col-lg-6:last-child .card-body');
                                    if (rightPanelContent) {
                                        const hasSubjectData = !rightPanelContent.textContent.includes('Chọn một ngành học');
                                        debugLog(`📊 Right panel has subject data: ${hasSubjectData}`);
                                        debugLog(`📊 Right panel text sample: "${rightPanelContent.textContent.substring(0, 100)}..."`);
                                    }
                                    
                                    // Look for any buttons in table
                                    const tableButtons = document.querySelectorAll('table button, tbody button, tr button, td button');
                                    debugLog(`🔘 Buttons in table area: ${tableButtons.length}`);
                                    
                                    if (tableButtons.length > 0) {
                                        tableButtons.forEach((btn, index) => {
                                            debugLog(`  Table button ${index}: "${btn.className}" text="${btn.textContent.trim()}"`);
                                        });
                                    }
                                };
                            }
                            
                            const testDeleteButtonsBtn = document.getElementById('testDeleteButtons');
                            if (testDeleteButtonsBtn) {
                                testDeleteButtonsBtn.onclick = function() {
                                    debugLog('=== DELETE BUTTON SEARCH ===');
                                    ...
                                    selectors.forEach(selector => {
                                        ...
                                        debugLog(`🔍 "${selector}": ${buttons.length} found`);
                                        
                                        if (buttons.length > 0) {
                                            buttons.forEach((btn, index) => {
                                                debugLog(`  [${index}] class="${btn.className}" data-id="${btn.dataset.id}" data-code="${btn.dataset.code}"`);
                                            });
                                        }
                                    });
                                    ...
                                    debugLog(`📍 Current URL: ${url.pathname}`);
                                    debugLog(`📍 Query params: ${url.search}`);
                                    
                                    // Check if we're on subjects view
                                    const hasSubjects = document.querySelector('.table') !== null;
                                    debugLog(`📋 Has subjects table: ${hasSubjects}`);
                                };
                            }
                            ...
                            if (testModalBtn) {
                                testModalBtn.onclick = function() {
                                    try {
                                        ...
                                        debugLog(`❌ Modal error: ${e.message}`);
                                    }
                                };
                            }
                            
                            const testDirectClickBtn = document.getElementById('testDirectClick');
                            if (testDirectClickBtn) {
                                testDirectClickBtn.onclick = function() {
                                    const deleteButtons = document.querySelectorAll('.delete-subject-btn');
                                    if (deleteButtons.length > 0) {
                                        const firstBtn = deleteButtons[0];
                                        debugLog(`🎯 Simulating click on first delete button (${firstBtn.dataset.code})`);
                                        firstBtn.click();
                                    } else {
                                        debugLog('❌ No delete buttons found to click!');
                                    }
                                };
                            }
                            ...
                            document.addEventListener('click', function (e) {
                                ...
                                console.log('Closest delete-subject-btn:', e.target.closest('.delete-subject-btn'));
                                ...
                                // Delete Subject - Removed duplicate handler, using manual binding at end of file
                            });
                        ...
                        // Bind delete buttons manually since event delegation isn't working
                        setTimeout(function () {
                            console.log('=== BINDING DELETE BUTTONS (1 second delay) ===');
                            const allButtons = document.querySelectorAll('button');
                            const deleteButtons = document.querySelectorAll('.delete-subject-btn');
                            ...
                            allButtons.forEach((btn, index) => {
                                if (btn.className.includes('delete') || btn.className.includes('subject')) {
                                    console.log(`Button ${index}: classes="${btn.className}", id="${btn.dataset.id}"`);
                                }
                            });

                            deleteButtons.forEach((btn, index) => {
                                console.log('Binding button', index, 'with ID:', btn.dataset.id);
                                ...
                                btn.onclick = function (e) {
                                    ...
                                    const id = this.dataset.id;
                                    const code = this.dataset.code;
                                    console.log('Delete clicked - ID:', id, 'Code:', code);
                                    ...
                                    // Check if we're viewing all majors or a specific major
                                    const isViewAll = new URLSearchParams(window.location.search).get('viewAll') === 'true';
                                    const selectedMajorId = '${selectedMajorId}';

                                    console.log('Context - isViewAll:', isViewAll, 'selectedMajorId:', selectedMajorId);
                                    ...
                                    if (isViewAll || !selectedMajorId || selectedMajorId === '' || selectedMajorId === 'null') {
                                        ...
                                        message.innerHTML = 'Bạn có chắc chắn muốn <strong>xóa hoàn toàn</strong> môn học <strong>' + code + '</strong>? <br><small class="text-danger">Lưu ý: Môn học sẽ bị xóa khỏi tất cả ngành học.</small>';
                                        button.textContent = 'Xóa hoàn toàn';
                                        button.className = 'btn btn-danger';
                                    } else {
                                        // Remove subject from major only
                                        ...
                                        document.getElementById('deleteSubjectIdParam').value = id;
                                        message.innerHTML = 'Bạn có chắc chắn muốn <strong>gỡ môn học</strong> <strong>' + code + '</strong> khỏi ngành này? <br><small class="text-info">Lưu ý: Môn học sẽ chỉ bị gỡ khỏi ngành này, không bị xóa hoàn toàn.</small>';
